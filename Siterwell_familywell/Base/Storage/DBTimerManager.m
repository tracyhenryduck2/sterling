...
- (void)createTimerTable{
    NSString *sql = [NSString stringWithFormat:@"create table if not exists %@(timerid varchar(5),enable integer,sid varchar(5),week varchar(5),hour varchar(5),min varchar(5),code varchar(20),devTid varchar(30),primary key(timerid,devTid))", timertable];
...
- (NSMutableArray *)queryAllTimers:(NSString *)devTid{
    ...
    [[DBManager sharedInstanced].dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([rs next]) {
            ...
            timermodel.enable = [rs intForColumn:@"enable"];
            timermodel.timerid = [rs stringForColumn:@"timerid"];
            timermodel.sid = [rs stringForColumn:@"sid"];
...
- (void)updateTimerEnable:(NSInteger)enable withTimerid:(NSString *)timerid withDevTid:(NSString *)DevTid {
    ...
    [[DBManager sharedInstanced].dbQueue inDatabase:^(FMDatabase *db) {
        NSString *sql = [NSString stringWithFormat:@"update %@ set enable = %d where timerid = '%@' and devTid = '%@'", timertable,(int)enable,timerid,DevTid];
        [db executeUpdate:sql];
    }];
}
