...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    return [self.model.itemsDataArr count];;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (![itemData isKindOfClass:[NSArray class]]){
        ...
        if (indexPath.row == _model.itemsDataArr.count-1) {
            cell.islast = YES;
        }
        return cell;
...
- (void)collectionView:(UICollectionView *)collectionView moveItemAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath*)destinationIndexPath{
    ...
    id sourceObject = [self.model.itemsDataArr objectAtIndex:sourceIndexPath.row];
    [[self.model mutableArrayValueForKey:@"itemsDataArr"] removeObjectAtIndex:sourceIndexPath.row];
    [[self.model mutableArrayValueForKey:@"itemsDataArr"]  insertObject:sourceObject atIndex:destinationIndexPath.row];
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    float height = [[UIScreen mainScreen]bounds].size.width /3;
...
- (void)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout beginGroupForItemAtIndexPath:(NSIndexPath *)itemIndexPath toGroupIndexPath:(NSIndexPath *)groupIndexPath selectedSnapShotView:(UIView *)snaptShotView{
    ...
    id groupData = [self.model.itemsDataArr objectAtIndex:groupIndexPath.row];
    ...
    id itemData = [self.model.itemsDataArr objectAtIndex:itemIndexPath.row];
    ...
    [self openGroupWithData:tempGroupData withSnapShotView:snaptShotView completion:^(BOOL finished) {
        ...
        //分组打开后，之前选中的item删除掉
        [[self.model mutableArrayValueForKey:@"itemsDataArr"] removeObject:itemData];
...
- (void)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout addItemAtIndexPath:(NSIndexPath *)itemIndexPath unOpenGroupAtIndexPath:(NSIndexPath *)groupIndexPath{
    ...
    [[self.model mutableArrayValueForKey:@"itemsDataArr"] replaceObjectAtIndex:groupIndexPath.row withObject:tempGroupArr];
...
- (void)cancelGroupInGroupViewWithItemData:(id)itemData withGroupData:(NSArray *)groupItemData withSnapShotView:(UIView *)snapShotView{
    ...
    [[self.model mutableArrayValueForKey:@"itemsDataArr"] insertObject:itemData atIndex:self.groupIndexPath.row];
    ...
    if (self.isGroupIndexOriginalIsGroup){
        if (!self.groupIndexPath) {
            if (groupItemData.count == 0) {
                [[self.model mutableArrayValueForKey:@"itemsDataArr"] replaceObjectAtIndex:[self.model mutableArrayValueForKey:@"itemsDataArr"].count-1 withObject:groupItemData];
            }else{
                [[self.model mutableArrayValueForKey:@"itemsDataArr"] replaceObjectAtIndex:selectedIndexPath.row+1 withObject:groupItemData];
            }
        }else{
            [[self.model mutableArrayValueForKey:@"itemsDataArr"] replaceObjectAtIndex:self.groupIndexPath.row withObject:groupItemData];
        }
    }else{
        //如果分组前， groupitem本身不是个分组，则取消退出来后，也要不是个分组
        [[self.model mutableArrayValueForKey:@"itemsDataArr"] replaceObjectAtIndex:self.groupIndexPath.row withObject:[groupItemData objectAtIndex:0]];
    }
    ...
    [self.collectionView performBatchUpdates:^{
        ...
        if ([groupItemData isKindOfClass:[NSArray class]] && [groupItemData count] == 0){
            if (!self.groupIndexPath) {
                [[self.model mutableArrayValueForKey:@"itemsDataArr"] removeObjectAtIndex:selectedIndexPath.row + 1];

            }else{
                [[self.model mutableArrayValueForKey:@"itemsDataArr"] removeObjectAtIndex:self.groupIndexPath.row];
            }
...
- (void)openGroupWithData:(NSArray *)itemDatas withSnapShotView:(UIView *)snapShotView completion:(void (^ __nullable)(BOOL finished))completion {
    ...
    groupMainView.subvc = self.subVC;
    ...
    self.subVC.tabBarController.tabBar.alpha = 0.6;
    self.subVC.navigationController.navigationBar.alpha = 0.4;
//    [[UIApplication sharedApplication].keyWindow addSubview:groupMainView];
...
- (void)closeGroupMainView:(UIView *)groupMainView completion:(void (^ __nullable)(BOOL finished))completion{
    ...
    [UIView animateWithDuration:0.3 delay:0.0 options:(UIViewAnimationOptionCurveLinear) animations:^{
        ...
        self.subVC.tabBarController.tabBar.alpha = 1;
        self.subVC.navigationController.navigationBar.alpha = 1;
